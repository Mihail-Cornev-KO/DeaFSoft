...
// Fonksiyonu: Bot bir bölgeye eklendiðinde bölge notice gösterimi eklenmiþtir.
void CBot::AddToRegion(int16 new_region_x, int16 new_region_z)
{
	// Eski bölgeden kaldýr
	if (GetRegion())
	...
	// Notice metni: Bot ismi ve yeni bölge koordinatlarýný gösterir
	std::string notice = string_format("%s Adlý Bot (%d, %d) Bölgesine Eklendi.", GetName(), new_region_x, new_region_z);
...
{
	// Oyun dýþýna çýkma durumunda gerekli kontrolleri yap
	if (bType == INOUT_OUT)
	{
	...
	if (bType == INOUT_OUT)
	{
		...
		// Belirli haritalarda özel iþlemler yapýlýyor
		if (pMap->GetID() == ZONE_ARDREAM ||
			pMap->GetID() == ZONE_RONARK_LAND ||
			pMap->GetID() == ZONE_BORDER_DEFENSE_WAR)
		{
			// Loyalty sýfýrlama ve oyuncu öldürme bölgesi sýralamasýndan silme
			m_iLoyaltyDaily = 0;
		...
		{
			// Kaos geniþleme sýralamasýndan silme
		...
		// Bölgedeki botlarý güncelle
		BotsSurroundingUserRegionUpdate();

		// Bölgeden kaldýrma iþlemi
		if (GetRegion())
		{
			GetRegion()->Remove(this);
		}
		...
		// Veritabanýndan kullanýcýyý kaldýrma
		g_DBAgent.RemoveCurrentUser(GetName());

		// Bot kullanýcýsýný güncelleme
	...
	else
	{
		...
		// Veritabanýna kullanýcý ekleme
		g_DBAgent.InsertCurrentUser(GetName(), GetName());

		// Bot kullanýcýsýný yükleme
		//g_DBAgent.GetLoadBotUser(this);

		// Oyun baþlangýç zamanýný kaydetme
