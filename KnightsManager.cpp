...
void CKnightsManager::CreateKnights(CUser* pUser, Packet & pkt)
{
	...
	// DeaFSoft Yeni Klan Kurabilmek Notice Eklendi 01.01.2025
	if (ret_value == MinLevelCreateClan) // DeaFSoft Klan Kurabilmek Level Notice Eklendi 01.01.2025
	{
		g_pMain->SendYesilNotice(pUser, string_format("[Clan Bilgilendirme 1] = Clan Kurabilmek Ýçin En Az [%d] Level'niz Olmasý Gereklidir", g_pMain->ClanCreateMinLevel));
		return;
	}

	if (ret_value == HaveCoinsDont) // DeaFSoft Klan Kurabilmek Para Notice Eklendi 01.01.2025
	{
		g_pMain->SendYesilNotice(pUser, string_format("[Clan Bilgilendirme 2] = Clan Kurabilmek Ýçin En Az [%d] Para'niz Olmasý Gereklidir", g_pMain->ClanCreateCoins));
		return;
	}

	if (ret_value == MinNpError) // DeaFSoft Klan Kurabilmek NP Notice Eklendi 01.01.2025
	{
		g_pMain->SendYesilNotice(pUser, string_format("[Clan Bilgilendirme 3] = Clan Kurabilmek Ýçin En Az [%d] NP'niz Olmasý Gereklidir", g_pMain->ClanCreateMinNationalPoints));
		return;
	}
	// DeaFSoft Yeni Klan Kurabilmek Notice Eklendi The End 01.01.2025
