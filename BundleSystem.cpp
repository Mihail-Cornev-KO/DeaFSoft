...
void CUser::DeaFSoftAutoLooter(_LOOT_BUNDLE* pBundle) {
...
#pragma endregion
	...
	for (int i = 0; i < NPC_HAVE_ITEM_LIST; i++) {
		...
		if (pBundle->Items[i].nItemID == ITEM_GOLD) {
			...
			// DeaFSoft Yeni İtem Türüne Göre Renk Atama Cok GüzeL Eklendi. 24.04.2025
			if (pTable.m_isDropNotice && g_pMain->pServerSetting.DropNotice && !isGM())
			{
				...
				// DeaFSoft İtem Türüne Göre Renk Atama Eklendi. 24.04.2025
				...
				// DeaFSoft LogosYolla Fonksiyonunu Çağırıyoruz, Renkle Mesajı Gönderiyoruz
