...
void CUser::Initialize()
{
	...
	m_oldTargetID = -1;
	m_ismsevent = false;
	GMUpgradeThis = false; // DeaFSoft GM Full Upgrade %100 Özel Eklendi 05.01.2025
	...
	// DeaFSoft Yeni Cz Pvp Kill Ödül Notice Eklendi. 12.04.2025
	m_PvpKillCount = 0;
	m_PvpKill50Count = 0;
	m_PvpKill500Count = 0;
	// DeaFSoft Yeni Cz Pvp Kill Ödül Notice Eklendi. The End. 12.04.2025
...
void CUser::OnDisconnect()
{
	...
	if (g_pMain->pServerSetting.GmisOnlineNotice) // DeaFSoft Giriş Gm Offline Notice Eklendi 30.12.2024
		DeaFSoftGmisOfflineNotice();
...
{
	if (GetPremium()) // DeaFSoft Pre Zone Online Kalacak Ödül Veriyoruz.
	{
		// DeaFSoft Yeni Zone Online Pre İtem Name Notice Eklendi. 12.04.2025
		_ITEM_TABLE pTable = g_pMain->GetItemPtr(pReward.pre_itemid);

		if (pReward.pre_itemid && pReward.pre_itemcount && !isMerchanting())
			GiveItem("PreZoneOnline", pReward.pre_itemid, pReward.pre_itemcount, true, pReward.pre_itemtime);
		if (pReward.pre_cash || pReward.pre_tl)
			GiveBalance(pReward.pre_cash, pReward.pre_tl);
		if (pReward.pre_loyalty)
			SendLoyaltyChange("PreZoneOnline", pReward.pre_loyalty);
		
		ShowEffect(490092); // DeaFSoft Yılbaşi Yağiyor Effect Eklendi. 13.04.2025
		Packet DeaFSoft; // DeaFSoft Public Chat Renk Sari Notice Eklendi 22.02.2025
		std::string PubliChat_Notice = string_format("Pre İtem Name : [ %s x%d ] | Pre Süre : [ %d ] | Pre Cash : [ %d ] | Pre TL : [ %d ] | Pre NP : [ %d ] | Pre Ödül İtem Gönderildin Tebrikler.", pTable.m_sName.c_str(), pReward.pre_itemcount, pReward.pre_itemtime, pReward.pre_cash, pReward.pre_tl, pReward.pre_loyalty);
		DeaFSoft.clear();
		ChatPacket::Construct(&DeaFSoft, ChatType::PUBLIC_CHAT, PubliChat_Notice.c_str(), "[Zone Online Pre Reward]", GetNation());
		Send(&DeaFSoft);
	}
	else
	// DeaFSoft Normal Zone Online Kalacak Ödül Veriyoruz.
	{
		// DeaFSoft Yeni Zone Online İtem Name Notice Eklendi. 12.04.2025
		_ITEM_TABLE pTable = g_pMain->GetItemPtr(pReward.itemid);
		...
		ShowEffect(490092); // DeaFSoft Yılbaşi Yağiyor Effect Eklendi. 13.04.2025
		Packet DeaFSoft; // DeaFSoft Public Chat Renk Mavi Notice Eklendi 22.02.2025
		std::string PubliChat_Notice = string_format("İtem Name : [ %s x%d ] | Süre : [ %d ] | Cash : [ %d ] | TL : [ %d ] | NP : [ %d ] | Ödül İtem Gönderildin Tebrikler.",pTable.m_sName.c_str(), pReward.itemcount, pReward.itemtime, pReward.cash, pReward.tl, pReward.loyalty);
		DeaFSoft.clear();
		ChatPacket::Construct(&DeaFSoft, ChatType::FORCE_CHAT, PubliChat_Notice.c_str(), "[Zone Online Reward]", GetNation());
		Send(&DeaFSoft);
	}
}
...
void CUser::SetSlotItemValue()
{
	...
	// DeaFSoft Yeni Süre İtemler Deleted Notice Eklendi. 06.02.2025 
	DeaFSoftExprationItemDeleted();
}
...
#define TOWN_TİME 1200
	...
	// DeaFSoft User Town Effect Eklendi 01.01.2025
