...
void CUser::RecvZoneChange(Packet & pkt)
{
	...
	if (opcode == ZoneChangeLoading)
	{
		...
		// DeaFSoft Giris Gate Zone Notice Eklendi. 04.01.2025
		if (g_pMain->GiriþGateZoneNotice)
			DeaFSoftGateZoneNotice();
	...
	else if (opcode == ZoneChangeLoaded)
	{
		...
		if (hasPriestBot == true)
		{
			...
			hasPriestBot = false;
			RemoveSavedMagic(495146);// sleave priest aynadan geçince fix DeaFSezo
		...
		// DeaFSoft User Zone Change Effect Eklendi 01.01.2025
