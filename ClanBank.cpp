...
void CUser::ClanWarehouseOpen(Packet& pkt)
{
	...
	if (g_pMain->pServerSetting.ClanBankWithPremium)
	{
		...
		{
			printf("DeaFSoft  | 1: %s: m_bIsLoggingOut=%d,sClanPremStatus=%d,g_pMain->ClanBankStatus=%d\n", __FUNCTION__, m_bIsLoggingOut, sClanPremStatus, g_pMain->ClanBankStatus);
			ReturnValue = 0;
			goto fail_return;
		}
	}
	else
	{
		...
		{
			printf("DeaFSoft  | 2: %s: m_bIsLoggingOut=%d,sClanPremStatus=%d,g_pMain->ClanBankStatus=%d\n", __FUNCTION__, m_bIsLoggingOut, sClanPremStatus, g_pMain->ClanBankStatus);
			ReturnValue = 0;
			goto fail_return;
		}
	}
