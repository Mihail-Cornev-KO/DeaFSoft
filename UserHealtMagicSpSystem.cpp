...
void CUser::OnDeath(Unit* pKiller)
{
	...
	// DeaFSoft User Deatch Effect Eklendi 01.01.2025
...
void CUser::OnDeathKilledPlayer(CUser* pKiller)
{
	...
	if (pKiller != this)
	{
		...
		// DeaFSoft Yeni Cz User Pvp Kill Ödül Notice Eklendi. 16.04.2025
		if (!pKiller->isInParty())
		{
		...
		// DeaFSoft Yeni Pvp Kill Ekran Notice Eklendi.
		if (pKiller->m_PvpKillCount == 5) { g_pMain->SendYeniNoticeZone(pKiller, string_format("%s is on RAMPAGE !!! | Kill Count : %d", pKiller->GetName().c_str(), pKiller->m_PvpKillCount)); }
		if (pKiller->m_PvpKillCount == 10) { g_pMain->SendYeniNoticeZone(pKiller, string_format("%s just got a Triple Kill! | Kill Count : %d", pKiller->GetName().c_str(), pKiller->m_PvpKillCount)); }
		if (pKiller->m_PvpKillCount == 15) { g_pMain->SendYeniNoticeZone(pKiller, string_format("%s just Drew First Blood Combisar! | Kill Count : %d ", pKiller->GetName().c_str(), pKiller->m_PvpKillCount)); }
		if (pKiller->m_PvpKillCount == 20) { g_pMain->SendYeniNoticeZone(pKiller, string_format("%s are OWNING! | Kill Count : %d ", pKiller->GetName().c_str(), pKiller->m_PvpKillCount)); }
		if (pKiller->m_PvpKillCount == 25) { g_pMain->SendYeniNoticeZone(pKiller, string_format("%s just got a Ultra Kill! | Kill Count : %d ", pKiller->GetName().c_str(), pKiller->m_PvpKillCount)); }
		if (pKiller->m_PvpKillCount == 30) { g_pMain->SendYeniNoticeZone(pKiller, string_format("%s Killing Spree! | Kill Count : %d", pKiller->GetName().c_str(), pKiller->m_PvpKillCount)); }
		if (pKiller->m_PvpKillCount == 35) { g_pMain->SendYeniNoticeZone(pKiller, string_format("%s Trouble-maker! | Kill Count : %d", pKiller->GetName().c_str(), pKiller->m_PvpKillCount)); }

		if (pKiller->m_PvpKillCount == 5)
		{
			// DeaFSoft Yeni KillÝtemGift Ýtem Name Notice Eklendi. 16.04.2025
			_ITEM_TABLE pTable = g_pMain->GetItemPtr(g_pMain->KillÝtemGift);

			pKiller->GoldGain(g_pMain->KillGoldGift);
			pKiller->CashGain(g_pMain->KillCashGift);
			//pKiller->SendLoyaltyChange("Pvp Kill Ödül", g_pMain->KillNpGift); // Np Gerek Yok.
			pKiller->GiveItem("Pvp Kill Ödül", g_pMain->KillÝtemGift, g_pMain->KillCountGift);
			pKiller->m_PvpKillCount = 0;
			pKiller->ShowEffect(490092); // DeaFSoft Yýlbaþi Yaðiyor Kazandin Akar :D Effect Eklendi. 16.04.2025
			PlayerEffect(31088); // DeaFSoft Yeni Kýrmýz Öldü Kaybetsin Effect Eklendi. 16.04.2025
			SendChat((uint8)ChatType::SHOUT_CHAT, string_format("Vs Öldün Tüh! o Ýntikam Bu Nick : [ %s ] Saldirsana.", pKiller->GetName().c_str()), "[Pvp Kill Ödül Kaybetsin]");
			g_pMain->SendHelpDescription(pKiller, string_format("[Pvp Kill Ödül Kazandin] = [ Para : %d ] | [ Cash : %d ] | [ Ýtem Name : %s x%d ] | [ Kill Count : %d ] Öldürme Görevi Bitirdin Ödül Ýtem Gönderildin Tebrikler. ", g_pMain->KillGoldGift, g_pMain->KillCashGift, pTable.m_sName.c_str(), g_pMain->KillCountGift, pKiller->m_PvpKillCount+5));
		}

		if (pKiller->m_PvpKill50Count == 50)
		{
			// DeaFSoft Yeni KillÝ50temGift Ýtem Name Notice Eklendi. 16.04.2025
			_ITEM_TABLE pTable = g_pMain->GetItemPtr(g_pMain->Kill50ÝtemGift);

			pKiller->GoldGain(g_pMain->Kill50GoldGift);
			pKiller->CashGain(g_pMain->Kill50CashGift);
			//pKiller->SendLoyaltyChange("Pvp Kill Ödül", g_pMain->Kill50NpGift);
			pKiller->GiveItem("Pvp Kill Ödül", g_pMain->Kill50ÝtemGift, g_pMain->Kill50CountGift);
			pKiller->m_PvpKillCount = 0;
			pKiller->m_PvpKill50Count = 0;
			pKiller->ShowEffect(490092); // DeaFSoft Yýlbaþi Yaðiyor Kazandin Akar :D Effect Eklendi. 16.04.2025
			PlayerEffect(31088); // DeaFSoft Yeni Kýrmýz Öldü Kaybetsin Effect Eklendi. 16.04.2025
			SendChat((uint8)ChatType::SHOUT_CHAT, string_format("Vs Öldün Tüh! o Ýntikam Bu Nick : [ %s ] Saldirsana.", pKiller->GetName().c_str()), "[Pvp Kill Ödül Kaybetsin]");
			g_pMain->SendHelpDescription(pKiller, string_format("[Pvp Kill Ödül Kazandin] = [ Para : %d ] | [ Cash : %d ] | [ Ýtem Name : %s x%d ] | [ Kill Count : %d ] Öldürme Görevi Bitirdin Ödül Ýtem Gönderildin Tebrikler. ", g_pMain->Kill50GoldGift, g_pMain->Kill50CashGift, pTable.m_sName.c_str(), g_pMain->Kill50CountGift, pKiller->m_PvpKill50Count+50));
		}

		if (pKiller->m_PvpKill500Count == 500)
		{
			// DeaFSoft Yeni Kill500ÝtemGift Ýtem Name Notice Eklendi. 16.04.2025
			_ITEM_TABLE pTable = g_pMain->GetItemPtr(g_pMain->Kill500ÝtemGift);

			pKiller->GoldGain(g_pMain->Kill500GoldGift);
			pKiller->CashGain(g_pMain->Kill500CashGift);
			//pKiller->SendLoyaltyChange("Pvp Kill Ödül", g_pMain->Kill500NpGift);
			pKiller->GiveWerehouseItem(g_pMain->Kill500ÝtemGift, g_pMain->Kill500CountGift);
			pKiller->m_PvpKillCount = 0;
			pKiller->m_PvpKill50Count = 0;
			...
			PlayerEffect(31088); // DeaFSoft Yeni Kýrmýz Öldü Kaybetsin Effect Eklendi. 16.04.2025
			SendChat((uint8)ChatType::SHOUT_CHAT, string_format("Vs Öldün Tüh! o Ýntikam Bu Nick : [ %s ] Saldirsana.", pKiller->GetName().c_str()), "[Pvp Kill Ödül Kaybetsin]");
			g_pMain->SendHelpDescription(pKiller, string_format("[Pvp Kill Ödül Kazandin] = [ Para : %d ] | [ Cash : %d ] | [ Ýtem Name : %s x%d ] | [ Kill Count : %d ] Öldürme Görevi Bitirdin Ödül Ýtem Gönderildin Tebrikler. ", g_pMain->Kill500GoldGift, g_pMain->Kill500CashGift, pTable.m_sName.c_str(), g_pMain->Kill500CountGift, pKiller->m_PvpKill500Count+500));
		}
			...
			if (pParty != nullptr)
			{
				for (int i = 0; i < MAX_PARTY_USERS; i++)
				{
					...
					if (pPartyUser)
					{
						...
						// DeaFSoft Yeni Pvp Kill Ekran Notice Eklendi.
						if (pPartyUser->m_PvpKillCount == 5) { g_pMain->SendYeniNoticeZone(pPartyUser, string_format("%s is on RAMPAGE !!! | Kill Count : %d", pPartyUser->GetName().c_str(), pPartyUser->m_PvpKillCount)); }
						if (pPartyUser->m_PvpKillCount == 10) { g_pMain->SendYeniNoticeZone(pPartyUser, string_format("%s just got a Triple Kill! | Kill Count : %d", pPartyUser->GetName().c_str(), pPartyUser->m_PvpKillCount)); }
						if (pPartyUser->m_PvpKillCount == 15) { g_pMain->SendYeniNoticeZone(pPartyUser, string_format("%s just Drew First Blood Combisar! | Kill Count : %d ", pPartyUser->GetName().c_str(), pPartyUser->m_PvpKillCount)); }
						if (pPartyUser->m_PvpKillCount == 20) { g_pMain->SendYeniNoticeZone(pPartyUser, string_format("%s are OWNING! | Kill Count : %d ", pPartyUser->GetName().c_str(), pPartyUser->m_PvpKillCount)); }
						if (pPartyUser->m_PvpKillCount == 25) { g_pMain->SendYeniNoticeZone(pPartyUser, string_format("%s just got a Ultra Kill! | Kill Count : %d ", pPartyUser->GetName().c_str(), pPartyUser->m_PvpKillCount)); }
						if (pPartyUser->m_PvpKillCount == 30) { g_pMain->SendYeniNoticeZone(pPartyUser, string_format("%s Killing Spree! | Kill Count : %d", pPartyUser->GetName().c_str(), pPartyUser->m_PvpKillCount)); }
						if (pPartyUser->m_PvpKillCount == 35) { g_pMain->SendYeniNoticeZone(pPartyUser, string_format("%s Trouble-maker! | Kill Count : %d", pPartyUser->GetName().c_str(), pPartyUser->m_PvpKillCount)); }
						...
						if (pPartyUser->m_PvpKillCount == 5)
						{
							...
							//pPartyUser->SendLoyaltyChange("Pvp Kill Ödül", g_pMain->KillNpGift);
							pPartyUser->GiveWerehouseItem(g_pMain->KillÝtemGift, g_pMain->KillCountGift);
							pPartyUser->m_PvpKillCount = 0;
							pPartyUser->ShowEffect(490092); // DeaFSoft Yýlbaþi Yaðiyor Kazandin Akar :D Effect Eklendi. 16.04.2025
							PlayerEffect(31088); // DeaFSoft Yeni Kýrmýz Öldü Kaybetsin Effect Eklendi. 16.04.2025
							SendChat((uint8)ChatType::SHOUT_CHAT, string_format("Vs Öldün Tüh! o Ýntikam Bu Nick : [ %s ] Saldirsana.", pKiller->GetName().c_str()), "[Pvp Kill Ödül Kaybetsin]");
							g_pMain->SendHelpDescription(pPartyUser, string_format("[Pvp Party Kill Ödül Kazandin] = [ Para : %d ] | [ Cash : %d ] | [ Ýtem Name : %s x%d ] | [ Kill Count : %d ] Öldürme Görevi Bitirdin Ödül Ýtem Gönderildin Tebrikler. ", g_pMain->KillGoldGift, g_pMain->KillCashGift, pTable.m_sName.c_str(), g_pMain->KillCountGift, pPartyUser->m_PvpKillCount+5));
						}

						if (pPartyUser->m_PvpKill50Count == 50)
						{
							// DeaFSoft Yeni Kill50ÝtemGift Ýtem Name Notice Eklendi. 16.04.2025
							_ITEM_TABLE pTable = g_pMain->GetItemPtr(g_pMain->Kill50ÝtemGift);

							pPartyUser->GoldGain(g_pMain->Kill50GoldGift);
							pPartyUser->CashGain(g_pMain->Kill50CashGift);
							//pPartyUser->SendLoyaltyChange("Pvp Kill Ödül", g_pMain->Kill50NpGift);
							pPartyUser->GiveWerehouseItem(g_pMain->Kill50ÝtemGift, g_pMain->Kill50CountGift);
							pPartyUser->m_PvpKillCount = 0;
							pPartyUser->m_PvpKill50Count = 0;
							pPartyUser->ShowEffect(490092); // DeaFSoft Yýlbaþi Yaðiyor Kazandin Akar :D Effect Eklendi. 16.04.2025
							PlayerEffect(31088); // DeaFSoft Yeni Kýrmýz Öldü Kaybetsin Effect Eklendi. 16.04.2025
							SendChat((uint8)ChatType::SHOUT_CHAT, string_format("Vs Öldün Tüh! o Ýntikam Bu Nick : [ %s ] Saldirsana.", pKiller->GetName().c_str()), "[Pvp Kill Ödül Kaybetsin]");
							g_pMain->SendHelpDescription(pPartyUser, string_format("[Pvp Party Kill Ödül Kazandin] = [ Para : %d ] | [ Cash : %d ] | [ Ýtem Name : %s x%d ] | [ Kill Count : %d ] Öldürme Görevi Bitirdin Ödül Ýtem Gönderildin Tebrikler. ", g_pMain->Kill50GoldGift, g_pMain->Kill50CashGift, pTable.m_sName.c_str(), g_pMain->Kill50CountGift, pPartyUser->m_PvpKill50Count+50));
						}
						...
						if (pPartyUser->m_PvpKill500Count == 500)
						{
							...
							//pPartyUser->SendLoyaltyChange("Pvp Kill Ödül", g_pMain->Kill500NpGift);
							pPartyUser->GiveWerehouseItem(g_pMain->Kill500ÝtemGift, g_pMain->Kill500CountGift);
							pPartyUser->m_PvpKillCount = 0;
							pPartyUser->m_PvpKill50Count = 0;
							...
							PlayerEffect(31088); // DeaFSoft Yeni Kýrmýz Öldü Kaybetsin Effect Eklendi. 16.04.2025
							SendChat((uint8)ChatType::SHOUT_CHAT, string_format("Vs Öldün Tüh! o Ýntikam Bu Nick : [ %s ] Saldirsana.", pKiller->GetName().c_str()), "[Pvp Kill Ödül Kaybetsin]");
							g_pMain->SendHelpDescription(pPartyUser, string_format("[Pvp Party Kill Ödül] = [ Para : %d ] | [ Cash : %d ] | [ Ýtem Name : %s x%d ] | [ Kill Count : %d ] Öldürme Görevi Bitirdin Ödül Ýtem Gönderildin Tebrikler. ", g_pMain->Kill500GoldGift, g_pMain->Kill500CashGift, pTable.m_sName.c_str(), g_pMain->Kill500CountGift, pPartyUser->m_PvpKill500Count+500));
						}
					}
				}
			}
		}
		// DeaFSoft Yeni Cz User Pvp Kill Ödül Notice Eklendi. The End. 16.04.2025
}
