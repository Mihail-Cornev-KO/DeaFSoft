...
void CUser::HandleBottomUserInfoDetail(Packet& pkt)
{
	...
	if (pUser != nullptr)
	{
		...
		// DeaFSoft Bottom User List View Notice Eklendi. 02.02.2025
		if (isGM() || isGMUser()) // Gm View Ýzle Ýzin Yok Yasak Bakma Sanane Aq :D
		{
			...
			// DeaFSoft UserInfoView Notice Eklendi. 02.02.2025
			...
			SendChat(FORCE_CHAT, string_format("<< Sol Np : [ %d ] | Sað Np : [ %d ] >>", pUser->GetLoyalty(), pUser->GetMonthlyLoyalty()));
			SendChat(KNIGHTS_CHAT, string_format("<< Cash Point : [ %d ] | TL : [ %d ] | Para : [ %d ] >>", pUser->GetCash(), pUser->m_nTLBalance, pUser->GetCoins()));

			// -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

			DateTime time;
			g_pMain->SendHelpDescription(this, string_format("<< Kullanýcýnýn Öldürme ve Ölme Sayisi 0 Oldugu Icin Kd Orani Hesaplanamadi >>"));
			g_pMain->SendHelpDescription(this, string_format("<< Toplam Öldürme Sayýsý : [ %d ] | Toplam Ölme Sayýsý : [ %d ] >>", pUser->m_AchieveInfo.UserDefeatCount, pUser->m_AchieveInfo.UserDeathCount));
			g_pMain->SendHelpDescription(this, string_format("<< Monster Öldürme Sayýsý : [ %d ] >>", pUser->m_AchieveInfo.MonsterDefeatCount));
		...
		// DeaFSoft Bottom User List View Notice Eklendi. The End 02.02.2025
	...
	else
	{
		...
		if (pBot != nullptr)
		{
			...
			// DeaFSoft Bottom Bot List View Notice Eklendi. 02.02.2025
			if (isGM() || isGMUser()) // Gm View Ýzle Ýzin Yok Yasak Bakma Sanane Aq :D
			{
				// DeaFSoft BotInfoView Notice Eklendi. 02.02.2025
				...
				SendChat(FORCE_CHAT, string_format("<< Sol Np : [ %d ] | Sað Np : [ %d ] >>", pBot->GetLoyalty(), pBot->GetMonthlyLoyalty()));
			...
			// DeaFSoft Bottom Bot List View Notice Eklendi. The End 02.02.2025
		}
	}
}
