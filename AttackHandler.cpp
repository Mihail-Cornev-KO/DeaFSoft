...
void CUser::Regene(uint8 regene_type, uint32 magicid /*= 0*/)
{
	...
	if (magicid == 0)
	{
		...
		if (GetLoyalty() == 0 && (GetMap()->isWarZone() || isInSpecialEventZone() || isInPKZone() || cindirella || BIFROST_EVENT)) // DeaFSezo np 0 kaldý ise bifrost haritasýndan atma Eklendim 28.12.2024
			KickOutZoneUser();
	}

	// DeaFSoft User Rise Effect Eklendi 01.01.2025
