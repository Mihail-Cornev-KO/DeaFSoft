...
void CGameServerDlg::UpdateGameTime()
{
	...
	if (m_sSec != now.GetSecond())
	{
		...
		if (g_pMain->m_checktime <= 0)
		{
			...
			SIZE_T physMemUsedByMe = GetProcessMemoryUsage(); // Bellek kullanýmýný al
			std::string texts;
			texts = string_format("DeaFSoft Systems [Online :%d - Merchant :%d - Mining :%d - Fishing :%d - Genie :%d] || RAM: %llu MB",
