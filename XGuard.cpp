...
void CUser::XSafe_Support(Packet& pkt)
{
	...
	// DeaFSoft Notice Eklendi. 29.04.2025
	string noticeMessage = "Yeni Bir Destek Bildirimi Alýndý. Teþekkür Ederiz. Konu: " + subject + " - Tip: " + ReportType;
...
void CUser::XSafe_Merchant(Packet& pkt)
{
	...
	if (subCode == MERCHANT_ITEM_ADD)
	{
		...
		{
			g_pMain->SendHelpDescription(this, string_format("Bir Pazar Oluþturmak Için Minimum %d Knight Cash Girmeniz Gerekir.", minreqcash).c_str());
			goto fail_return;
		}
...
void CUser::XSafe_ChaoticExchange(Packet& pkt)
{
	...
	{
		g_pMain->SendHelpDescription(this, string_format("Deðiþim Ýþlemini Yapabilmek Için %d Adet Coin'e Ýhtiyacýnýz Var.", coinsreq));
		return BifrostPieceSendFail(errorcode);
	}

	if (count > 100)
	{
		g_pMain->SendHelpDescription(this, "Maksimum Parça Sayýsý 100.");
		return BifrostPieceSendFail(errorcode);
	}
	...
	if (!GeneratorCheckExistItemCount(nExchangeItemID, count)) {
		g_pMain->SendHelpDescription(this, "Yeterli Sayýda Eþyaya Sahip Deðilsiniz.");
		return BifrostPieceSendFail(errorcode);
	}
	else
	{
	...
	for (int i = 0; i < count; i++)
	{
		...
		if (bFreeSlots < 1)
		{
			g_pMain->SendHelpDescription(this, "Yeterli Alan Kalmadý.");
			return BifrostPieceSendFail(errorcode);
		}
		...
		{
			g_pMain->SendHelpDescription(this, "Envanterinizde Çok Fazla Aðýrlýk Var.");
			return BifrostPieceSendFail(errorcode);
		}
		...
		if (seslling)
		{
			...
			{
				g_pMain->SendHelpDescription(this, "Envanterinizde Çok Fazla Jeton Var.");
				return BifrostPieceSendFail(errorcode);
			}

			if (blocklistiteminfo)
				g_pMain->SendHelpDescription(this, string_format("Blok Durumundaki Öðenin Ürün Adý : %s", pGiveTable.m_sName.c_str()));
		...
		if (bank)
		{
			...
			{
				g_pMain->SendHelpDescription(this, "Bankanýzda Yeterli Alan Yok.");
				return BifrostPieceSendFail(errorcode);
			}
...
void CUser::XSafe_PUSPurchase(Packet& pkt)
{
	...
	{
		g_pMain->SendHelpDescription(this, "CindWar Eðlenceli Sýnýf Etkinliði Sýrasýnda Güçlendirme Deposunu Kullanamazsýnýz");
		return;
	}
	if (isMerchanting())
	{
		g_pMain->SendHelpDescription(this, "Pazar Tüccarlýk Yaparken Güçlendirme Maðazasýný Kullanamazsýnýz.");
		return;
	}

	if (isTrading())
	{
		g_pMain->SendHelpDescription(this, "Trade Ticaret Yaparken Güçlendirme Deposunu Kullanamazsýnýz.");
		return;
	}
	...
	if (!item->PriceType)
	{
		if (m_nKnightCash >= totalCash)
		{
			...
			{
				if (!GiveItem("Pus Geri Satýn Alma", item->ItemID, item->BuyCount, true, 0, _giveitempusinfo(true, item->PriceType, item->Price)))
					continue;

				m_nKnightCash -= item->Price;

				if (notice)
					g_pMain->SendHelpDescription(this, string_format("Tebrikler! Satýn Aldýðýnýz %s Ürün %d Knight Cash.", itemdata.m_sName.c_str(), item->Price).c_str());

				//#######################################################################################################################################################################

				// DeaFSoft Yeni PUS TL&KC item List Ýzleme Pm Notice Eklendi. 21.04.2025
				std::string PMName = "Pus KC&TL Ürün List";

				if (notice && !isGM() && !isGMUser()) // DeaFSoft Gmsiz ve GmUsersiz Notice Eklendi. 21.04.2025
				{
					std::string Mesaj = "----------------------------------------------";
					Packet result;
					ChatPacket::Construct(&result, ChatType::PRIVATE_CHAT, &Mesaj, &PMName);
					g_pMain->Send_All(&result); // DeaFSoft Pm Notice Herkes Görüyoruz. Eklendi. 21.04.2025

					{
						if (item->BuyCount > 1) // Pus Adet Eklendi.
							Mesaj = string_format("%s Ürün : %s x%d KC : %d Satýn Aldýn Tebrikler!", m_strUserID.c_str(), itemdata.m_sName.c_str(), item->BuyCount, item->Price).c_str();
						else
							Mesaj = string_format("%s Ürün : %s KC : %d Satýn Aldýn Tebrikler!", m_strUserID.c_str(), itemdata.m_sName.c_str(), item->Price).c_str();
					}

					ChatPacket::Construct(&result, ChatType::PRIVATE_CHAT, &Mesaj, &PMName);
					g_pMain->Send_All(&result); // DeaFSoft Pm Notice Herkes Görüyoruz. Eklendi. 21.04.2025
					// DeaFSoft Yeni PUS TL&KC item List Ýzleme Pm Notice Eklendi. The End 21.04.2025
				}
			}
				//#######################################################################################################################################################################
		...
		if (m_nTLBalance >= totalCash)
		{
			...
			for (size_t i = 1; i <= count; i++)
			{
				if (item->ItemID >= 489500000 && item->ItemID <= 489600000)
				{
					...
					result << uint8(XSafeOpCodes::CASHCHANGE) << uint32(m_nKnightCash) << uint32(m_nTLBalance);
					Send(&result);
					g_pMain->SendHelpDescription(this, string_format("Tebrikler! Satýn Aldýnýz Change %d TL Balance to %d Knight Cash", item->Price, item->BuyCount).c_str());
					return;
				}
				...
				if (notice)
					g_pMain->SendHelpDescription(this, string_format("Tebrikler! Satýn Aldýnýz %s Ürün %d TL Balance Cash.", itemdata.m_sName.c_str(), item->Price).c_str());
				...
				if (notice && !isGM() && !isGMUser()) // DeaFSoft Gmsiz ve GmUsersiz Notice Eklendi. 21.04.2025
				{
					...
					{
						if (item->BuyCount > 1) // Pus Adet Eklendi.
							Mesaj = string_format("%s Ürün : %s x%d TL : %d Satýn Aldýn Tebrikler!", GetName().c_str(), itemdata.m_sName.c_str(), item->BuyCount, item->Price).c_str();
						else
							Mesaj = string_format("%s Ürün : %s TL : %d Satýn Aldýn Tebrikler!", GetName().c_str(), itemdata.m_sName.c_str(), item->Price).c_str();
					}
...
void CUser::HandleRightClickExchange(Packet& pkt)
{
	...
	case RightClickExchangeSubcode::GetCurrentItemList:
	{
		...
		if (pData == nullptr)
		{
			g_pMain->SendHelpDescription(this, "ItemID Bulunamadý Veritabaný.");
			break;
		}
	...
	case RightClickExchangeSubcode::SendItemExchange:
	{
		...
		{
			g_pMain->SendHelpDescription(this, "Select ItemID Bulunamadý Veritabaný.");
			break;
		}
		if (pData->bSelection)
		{
		...
		else
		{
			...
			if (nItemID == ITEM_CLAN_PREMIUM)
			{
				if (!isClanLeader())
				{
					XSafe_SendMessageBox("Right Click Exchange", "Clan Baþkaný Deðil Ýseniz Kýrdýramazsýnýz");
					clanpreerror = true;
					break;
				}
