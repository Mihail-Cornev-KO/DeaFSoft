...
bool CDBAgent::LoadBotTable()
{
	...
	do
	{
		...
		dbCommand->FetchUInt32(field++, pBot->m_iLoyaltyMonthly);*///DeaFSezo burayý açtým 28.12.2024
...
uint8 CDBAgent::LoadUserAkaraHistory(std::vector<ByteBuffer>& List, std::string& UserName)
{
	...
	if (!dbCommand->Execute(string_format(_T("SELECT * FROM AKARA_LIST WHERE UserName = '%s'"), UserName.c_str())))
	{
		ReportSQLError(m_AccountDB->GetError());
		return false;
	}
...
uint8 CDBAgent::UpdateAkaraHistory(uint8 listeNum, uint8 sira, std::vector<AKARA_LIST>& List, uint8 sonuc)
{
	...
	AkaraList << listeNum << sira << uint8(sonuc) << List.at(listeNum).ItemNum[sira] << List.at(listeNum).BetMax[sira];
...
// DeaFSoft Welcome Logos Notice Eklendi 01.01.2025
#pragma region CDBAgent::LoadDeaFSoftWelcomeLogosNotice
void CDBAgent::LoadDeaFSoftWelcomeLogosNotice()
{
	...
	if (dbCommand->hasData())
	{
		...
		if (Status == 1)
			g_pMain->DeaFSoftWelcomeStatus = true;
		else
			g_pMain->DeaFSoftWelcomeStatus = false;
...
#pragma endregion
// DeaFSoft Welcome Logos Notice Eklendi The End 01.01.2025