...
void CUser::MerchantOpen()
{
	...
	if (isDead())
		...
		errorCode = MERCHANT_OPEN_INVALID_ZONE; // DeaFSezo maradon dýþý pazar kurma engellendi
...
void CUser::MerchantItemBuy(Packet& pkt)
{
	...
	if (m_sMerchantsSocketID < MAX_USER)
	{
		...
		if (pMerch->isKC)
		{
			...
			// DeaFSoft Yeni Pazarlar Para&KC List Ýzleme Pm Notice Eklendi. 25.04.2025
			...
			g_pMain->Send_All(&result); // DeaFSoft Pm Notice Herkes Görüyoruz. Eklendi. 25.04.2025

			ShowEffect(490092); // DeaFSoft Yýlbaþi Yaðiyor Effect Eklendi. 25.04.2025
			{
				if (item_count > 1) // Ýtem KC Adet Eklendi.
					Mesaj = string_format("%s -> %s [%s x%d] KC : [-%d] Aldýnýz Tebrikler!", GetName().c_str(), pMerchUser->GetName().c_str(), pSellingItem.m_sName.c_str(), item_count, req_gold);
				else
					Mesaj = string_format("%s -> %s [%s] KC : [-%d] Aldýnýz Tebrikler!", GetName().c_str(), pMerchUser->GetName().c_str(), pSellingItem.m_sName.c_str(), req_gold);
			}
		...
		else
		{
			...
			ShowEffect(490092); // DeaFSoft Yýlbaþi Yaðiyor Effect Eklendi. 25.04.2025
			{
				if (item_count > 1) // Ýtem Coins Adet Eklendi.
					Mesaj = string_format("%s -> %s [%s x%d] Coins : [-%d] Aldýnýz Tebrikler!", GetName().c_str(), pMerchUser->GetName().c_str(), pSellingItem.m_sName.c_str(), item_count, req_gold);
				else
					Mesaj = string_format("%s -> %s [%s] Coins : [-%d] Aldýnýz Tebrikler!", GetName().c_str(), pMerchUser->GetName().c_str(), pSellingItem.m_sName.c_str(), req_gold);
			}
...
void CUser::BuyingMerchantOpen(Packet& pkt)
{
	...
	if (isDead())
		...
		errorCode = MERCHANT_OPEN_TRADING;
	else if (GetZoneID() != ZONE_MORADON) // DeaFSezo maradon dýþý pazar kurma engellendi
