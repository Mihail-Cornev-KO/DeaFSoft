...
void CUser::ItemTrade(Packet& pkt)
{
	...
	if (type == 1 && pNpc->m_iSellingGroup != 249000)
	{
		for (int i = 0; i < purchased_item_count; i++) {
			...
			// DeaFSoft Yeni Shop Al-Sat item List Ýzleme Pm Notice Eklendi. 20.04.2025
			std::string PMName = "Shop Ýtem AL-Sat List";

			if (pItems[i].price > 999999) // DeaFSoft Para Sýnýrý : 1m Ayarladým. Shop Alýnýr. Arrow, Blood, Ts, vb 1 - 999k Ucuz Almaz | Shop Alýnýr 1m - 100m, vb Pahalý Alacaz Notice Ekledim. 20.04.2025
			{
				std::string Mesaj = "----------------------------------------------";
				Packet result;
				ChatPacket::Construct(&result, ChatType::PRIVATE_CHAT, &Mesaj, &PMName);
				g_pMain->Send_All(&result); // DeaFSoft Pm Notice Herkes Görüyoruz. Eklendi. 20.04.2025

				{
					if (pItems[i]._ICOUNT > 1) // Ýtem Adet Eklendi.
						Mesaj = string_format("[ %s ] | Ýtem Name : [ %s x%d ] | Para : [ %d ] Aldýn. [ Time - %d:%d:%d ]", GetName().c_str(), pTable.m_sName.c_str(), pItems[i]._ICOUNT, pItems[i].price, time.GetHour(), time.GetMinute(), time.GetSecond());
					else
						Mesaj = string_format("[ %s ] | Ýtem Name : [ %s ] | Para : [ %d ] Aldýn. [ Time - %d:%d:%d ]", GetName().c_str(), pTable.m_sName.c_str(), pItems[i].price, time.GetHour(), time.GetMinute(), time.GetSecond());
				}
	...
	else if (type == 2)
	{
		...
		for (int i = 0; i < purchased_item_count; i++) {
			...
			if (pItems[i].price > 999999) // DeaFSoft Para Sýnýrý : 1m Ayarladým. Shop Satti. Arrow, Blood, Ts, vb 1 - 999k Ucuz Almaz | Shop Satti 1m - 100m, vb Pahalý Alacaz Notice Ekledim. 20.04.2025
			{
				std::string Mesaj = "----------------------------------------------";
				Packet result;
				ChatPacket::Construct(&result, ChatType::PRIVATE_CHAT, &Mesaj, &PMName);
				...
				{
					if (pItems[i]._ICOUNT > 1) // Ýtem Adet Eklendi.
						 Mesaj = string_format("[ %s ] | Ýtem Name : [ %s x%d ] | Para : [ -%d ] Sattýn. [ Time - %d:%d:%d ]", GetName().c_str(), pTable.m_sName.c_str(), pItems[i]._ICOUNT, pItems[i].price, time.GetHour(), time.GetMinute(), time.GetSecond());
					else
						 Mesaj = string_format("[ %s ] | Ýtem Name : [ %s ] | Para : [ -%d ] Sattýn. [ Time - %d:%d:%d ]", GetName().c_str(), pTable.m_sName.c_str(), pItems[i].price, time.GetHour(), time.GetMinute(), time.GetSecond());
				}
