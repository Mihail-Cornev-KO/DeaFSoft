...
void CUser::SelectCharacter(Packet & pkt)
{
	...
	if ((pUser && pUser->m_bSelectedCharacter))
		return goDisconnect("DeaFSoft | DC [1].", __FUNCTION__);
	...
	case 4:
	{
		...
		if (strCharID1.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID1, ZoneID))
			{
				...
				{
					result << uint8(PrisonCharacter) << strCharID1 << "DeaFSoft - Keeper";
					Send(&result);
					return;
				}
			}
		...
		if (strCharID2.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID2, ZoneID))
			{
				if (ZoneID == ZONE_PRISON)
				{
					result << uint8(PrisonCharacter) << strCharID2 << "DeaFSoft - Keeper";
					Send(&result);
					return;
				}
			}
		...
		if (strCharID3.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID3, ZoneID))
			{
				if (ZoneID == ZONE_PRISON)
				{
					result << uint8(PrisonCharacter) << strCharID3 << "DeaFSoft - Keeper";
					Send(&result);
					return;
				}
			}
		...
		if (strCharID4.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID4, ZoneID))
			{
				if (ZoneID == ZONE_PRISON)
				{
					result << uint8(PrisonCharacter) << strCharID4 << "DeaFSoft - Keeper";
					Send(&result);
					return;
				}
			}
...
void CUser::GameStart(Packet& pkt)
{
	...
	else if (opcode == 2)
	{
		...
		// DeaFSoft Sistem Mesajları Renk Kırmızı Eklendi 02.01.2025
		...
		{ // DeaFSoft Sistem Mesajları Renk Sari Eklendi 02.01.2025
			std::vector< _SEND_MESSAGE> m_messagelist;
			g_pMain->m_SendMessageArray.m_lock.lock();
			foreach_stlmap_nolock(itr, g_pMain->m_SendMessageArray) {
				_SEND_MESSAGE* msg = itr->second;
		...
		{ // DeaFSoft Sistem Mesajları Renk Yeşil Eklendi 02.01.2025
			std::vector< _SEND_MESSAGE> m_messagelist;
			g_pMain->m_SendMessageArray.m_lock.lock();
			foreach_stlmap_nolock(itr, g_pMain->m_SendMessageArray) {
				_SEND_MESSAGE* msg = itr->second;
		...
		{ // DeaFSoft Sistem Mesajları Renk Pembe Eklendi 02.01.2025
			std::vector< _SEND_MESSAGE> m_messagelist;
			g_pMain->m_SendMessageArray.m_lock.lock();
			foreach_stlmap_nolock(itr, g_pMain->m_SendMessageArray) {
				_SEND_MESSAGE* msg = itr->second;
		...
		{ // DeaFSoft Sistem Mesajları Renk Mavi Eklendi 02.01.2025
			std::vector< _SEND_MESSAGE> m_messagelist;
			g_pMain->m_SendMessageArray.m_lock.lock();
			foreach_stlmap_nolock(itr, g_pMain->m_SendMessageArray) {
				_SEND_MESSAGE* msg = itr->second;
		...
		{ // DeaFSoft Sistem Mesajları Renk Mavi Açık Eklendi 02.01.2025
			std::vector< _SEND_MESSAGE> m_messagelist;
			g_pMain->m_SendMessageArray.m_lock.lock();
			foreach_stlmap_nolock(itr, g_pMain->m_SendMessageArray) {
				_SEND_MESSAGE* msg = itr->second;
		...
		// DeaFSoft Login Effect 01.01.2025
		if (g_pMain->UserLoginEffect)
		PlayerEffect(15005);

		// DeaFSoft Welcome Notice Status Eklendi 01.01.2025
		if (g_pMain->DeaFSoftWelcomeStatus)
			DeaFSoftWelcomeLogos("[DeaFSoft]", g_pMain->WelcomeNoticeYeni, g_pMain->WelcomeNoticeR, g_pMain->WelcomeNoticeG, g_pMain->WelcomeNoticeB);

		if (g_pMain->pServerSetting.OyunUserBotOnlineNotice) // DeaFSoft User&Bot Oyunda Online Notice Eklendi 31.12.2024
			DeaFSoftOyunOnlineNotice();

		if (g_pMain->pServerSetting.GmisOnlineNotice) // DeaFSoft Giriş Gm Online Notice Eklendi 30.12.2024
			DeaFSoftGmisOnlineNotice();

		//DeaFSoft Login User İsOnline Notice 01.01.2025
		//if (g_pMain->GirişUserisOnlineNotice)
			//DeaFSoftLoginUserisOnlineNotice();

		//DeaFSoft Gm Info Komut Notice 02.01.2025
		if (g_pMain->GmInfoKomutNotice)
			DeaFSoftGmInfoKomutNotice();

		// DeaFSoft Yeni Süre İtemler Info Notice Eklendi. 06.02.2025 
		DeaFSoftExprationItemList();
	...
	// DeaFSoft Auto King Name Update Eklendi 01.01.2025
	...
	// DeaFSoft Auto King Name Update Eklendi The End 01.01.2025
