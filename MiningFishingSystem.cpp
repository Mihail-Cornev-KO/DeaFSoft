...
void CUser::HandleMiningStart(Packet & pkt)
{
	...
	std::string PubliChat_Notice = string_format("Nick : %s | Mining Start : %d Baþladi. - Saat : %d:%d:%d", GetName().c_str(), m_bMining, time.GetHour(), time.GetMinute(), time.GetSecond());
	DeaFSoft.clear();
	ChatPacket::Construct(&DeaFSoft, ChatType::KNIGHTS_CHAT, PubliChat_Notice.c_str(), "[Mining Start]", GetNation());
	Send(&DeaFSoft);
}
...
void CUser::HandleMiningAttempt(Packet & pkt)
{
	...
	if (nItemID == ITEM_EXP) // DeaFSoft Maden, Golden ve Mattock Effect ve Exp Notice Eklendi. 22.02.2025
	{
		...
		DeaFSoft.clear();
		ChatPacket::Construct(&DeaFSoft, ChatType::PUBLIC_CHAT, PubliChat_Notice.c_str(), "[Mining Exp Bilgi]", GetNation());
		Send(&DeaFSoft);
	}
	else if (nItemID > 0) // DeaFSoft Maden, Golden ve Mattock Effect ve Ýtem Notice Eklendi. 22.02.2025
	{
		...
		sEffect = 13081; // ÝTEM EFFECT
		ShowEffect(490092); // Yýlbaþi Yaðiyor Effect Eklendi. 22.02.2025

		Packet DeaFSoft; // DeaFSoft Public Chat Renk Kýrmýzý Notice Eklendi 22.02.2025
		std::string PubliChat_Notice = string_format("Nick : %s | Mining Ýtem Name : %s Aldin Tebrikler", GetName().c_str(), pTable.m_sName.c_str());
		DeaFSoft.clear();
		ChatPacket::Construct(&DeaFSoft, ChatType::SHOUT_CHAT, PubliChat_Notice.c_str(), "[Mining Ýtem Bilgi]", GetNation());
		Send(&DeaFSoft);
	}
...
void CUser::HandleMiningStop()
{
	...
	DeaFSoft.clear();
	ChatPacket::Construct(&DeaFSoft, ChatType::SHOUT_CHAT, PubliChat_Notice.c_str(), "[Mining Stop]", GetNation());
	Send(&DeaFSoft);
}
...
void CUser::HandleFishingStart(Packet & pkt)
{
	...
	Packet DeaFSoft; // DeaFSoft Public Chat Renk Sari Notice Eklendi 22.02.2025
	std::string PubliChat_Notice = string_format("Nick : %s | Fishing Start : %d Baþladi. - Saat : %d:%d:%d", GetName().c_str(), m_bMining, time.GetHour(), time.GetMinute(), time.GetSecond());
	DeaFSoft.clear();
	ChatPacket::Construct(&DeaFSoft, ChatType::KNIGHTS_CHAT, PubliChat_Notice.c_str(), "[Fishing Start]", GetNation());
	Send(&DeaFSoft);
}
	...
	if (resultCode == MiningResultSuccess && g_pMain->m_MiningFishingItemArray.GetSize() > 0)
	{
		...
		if (pFishingList.size() > 0)
		{
			...
			else if (nItemID > 0)
			{
				...
				sItemSlot = GetEmptySlot() - SLOT_MAX;
				GiveItem("Fishing Ýtem", nItemID, 1);
				sEffect = 30730; // "Item" effect

				Packet DeaFSoft; // DeaFSoft Public Chat Renk Sari Notice Eklendi 06.02.2025
				std::string PubliChat_Notice = string_format("Nick : %s | Fishing Ýtem Name : %s Aldin Tebrikler", GetName().c_str(), pTable.m_sName.c_str());
				DeaFSoft.clear();
				ChatPacket::Construct(&DeaFSoft, ChatType::PUBLIC_CHAT, PubliChat_Notice.c_str(), "[Fishing Ýtem Bilgi]", GetNation());
				Send(&DeaFSoft);
			}
		}
...
void CUser::HandleFishingStop(Packet & pkt)
{
	...
	DeaFSoft.clear();
	ChatPacket::Construct(&DeaFSoft, ChatType::SHOUT_CHAT, PubliChat_Notice.c_str(), "[Fishing Stop]", GetNation());
	Send(&DeaFSoft);
}
