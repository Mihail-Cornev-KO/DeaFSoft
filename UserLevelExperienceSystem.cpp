...
void CUser::LevelChange(uint8 level, bool bLevelUp)
{
    ...
    // DeaFSoft Yeni Level Yüksek K&H List Ýzleme Pm Notice Eklendi. 22.04.2025
	//std::map<uint8, std::string> levelNotices =
	//{
	//	{ 5,  "Level 5 oldun! Baþlangýç artýk bitti." },
	//	{ 10, "Level 10 oldun! Yolun baþýndasýn." },
	//	{ 15, "Level 15 oldun! Güçleniyorsun." },
	//	{ 20, "Level 20 oldun! Artýk tehlikeler seni bekliyor." },
	//	{ 25, "Level 25 oldun! Harika gidiyorsun." },
	//	{ 30, "Level 30 oldun! Orta seviyelere ulaþtýn." },
	//	{ 35, "Level 35 oldun! Güçlü bir savaþçýsýn." },
	//	{ 40, "Level 40 oldun! Seni kimse tutamaz." },
	//	{ 45, "Level 45 oldun! Tehlikeli düþmanlara hazýr ol." },
	//	{ 50, "Level 50 oldun! Efsane olma yolundasýn." },
	//	{ 55, "Level 55 oldun! Harikalar diyarýna hoþ geldin." },
	//	{ 60, "Level 60 oldun! Artýk bir ustasýn." },
	//	{ 65, "Level 65 oldun! Saygý duyulan birisin." },
	//	{ 70, "Level 70 oldun! Destan yazmaya az kaldý." },
	//	{ 75, "Level 75 oldun! Zafere yaklaþtýn." },
	//	{ 80, "Level 80 oldun! Efsaneler arasýnda yerini al." },
	//	{ 83, "Level 83 oldun! Zirvedesin, tebrikler!" },
	//	{ 85, "Level 85 oldun! Maksimum seviyeye ulaþtýn!" }
	//};
...
bool CUser::JackPotNoah(uint32 gold)
{
	...
	if (rand_ == 500 || rand_ == 1000) {
		// DeaFSoft Jackpot Coin Patladý Notice Eklendi. 04.01.2025
		std::string notice = string_format("Oyuncu : %s Adlý Karakter Ýçin JackPot COIN Patladý %dx Hayirli Olsun.", GetName().c_str(), xrand);
		Packet result;
...
bool CUser::JackPotExp(int64 iExp)
{
	...
	if (xrand == 500 || xrand == 1000) {
		// DeaFSoft Jackpot EXP Patladý Notice Eklendi. 04.01.2025
		std::string notice = string_format("Oyuncu : %s Adlý Karakter Ýçin JackPot EXP Patladý %dx Hayirli Olsun.", GetName().c_str(), xrand);
		Packet result;
		ChatPacket::Construct(&result, (uint8)ChatType::GENERAL_CHAT, &notice);
		g_pMain->Send_All(&result, nullptr, (uint8)Nation::ALL);
	}
